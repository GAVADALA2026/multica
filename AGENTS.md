...
- Global Issues/My Issues include all projects by default; projects are ordinary filters. Status boards use independent lanes by the issue’s pinned workflow_id, with concrete status nodes and no cross-workflow dragging. Server group descriptors own lane membership, counts, and pagination; never derive lanes from loaded cards. Saved views own their query and preference key independently of external project selection.
